...
function fetchList(apiPath) {
  return new Promise(function ($return, $error) {
    let results, page, nextPage;

    results = [];
    page = 1;
    return getNext(apiPath, page).then(function ($await_3) {
      nextPage = $await_3;
      ...
      function $Loop_1() {
        if (nextPage.length) {
          ...
          page += 1;
          return getNext(apiPath, page).then(function ($await_4) {
            nextPage = $await_4;
            return $Loop_1;
...
function getNext(apiPath, page) {
  return new Promise(function ($return, $error) {
    var url;
    url = [_config.gitApiUrl, apiPath, '?per_page=100', '&access_token=' + String(_config.accessToken), '&page=' + String(page)].join('');
    return fetch(url, { method: 'GET' }).then($return, $error);
  }.$asyncbind(this));
}